...
def api_call_movie_list():
    ...
    params = "api_key=2103434b666fc1533fbb046838915b7c&include_adult=false&include_video=false&language=ko-KR&vote_count.gte=300&with_original_language=ko&sort_by=vote_average.desc&page="
    movie_data_list = []
    que = 1
    for i in range(1, 6):
        ...
        if response.status_code == 200:
            ...
            for obj in arr:
                    ...
                    for obj in arr2:
                        if "department" in obj:
                            ...
                            if obj["department"] == "Directing":  # 감독일 경우
                                movie_director = obj["name"]  # 감독 이름 저장
                        else:
                            continue
                ...
                movie_data['movie_content'] = movie_content
                movie_data['movie_open_date'] = movie_open_date
                movie_data['movie_vote'] = movie_vote
                movie_data['movie_poster_url'] = movie_poster_url
                movie_data['movie_runtime'] = movie_runtime
                movie_data['movie_country'] = movie_country
                movie_data['movie_director'] = movie_director
                movie_data['movie_actor'] = movie_actor
                movie_data['movie_genres'] = movie_genres
                root['model'] = model
                root['pk'] = pk
                root['field'] = movie_data

                    # print(movie_title)
                movie_data_list.append(root)
                que += 1
