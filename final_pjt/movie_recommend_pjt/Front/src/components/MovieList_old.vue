...
const getTopRatedMovies = () => {
  ...
  const endpoints = BASE_URL + '/movie/top_rated'
  const param = {
      api_key: API_KEY,
      language: 'ko-KR'
  }
  httpGetAxios(endpoints, param).then(res => {
      if(res.status === 200) {
        movieList.value = res.data.results  // 데이터를 movies 배열에 저장
      } else {
...
const endIndex = ref(3) // 한 번에 보여줄 카드의 개수를 1개로 설정
...
const moveToNextGenre = (index) => {
  const genre = movies.value[index]
  console.log(genre)
