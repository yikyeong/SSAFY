    ...
    <body>
        <div>
            <div v-if="result.length > 0">
                <DepositProductsAll 
                v-for="depositProduct in result" 
...
<script setup>
import DepositProductsAll from '@/components/Deposit/DepositProducts.vue'
// import DepositSearchForm from '@/components/Deposit/DepositSearchForm.vue'
...
const bankTypeFilter = function (bank, interest_type) {
    const result = []
    for (const product of store.depositProductsList) {
        const saving_list = []
...
if (bank.value === '전체 은행' && interest_type.value === '전체 금리 유형') {
    // 전체 조회
    result.value = store.depositProductsList
 } else if (bank.value !== '전체 은행' && interest_type.value === '전체 금리 유형') {
    // 은행만 필터링
    result.value = store.depositProductsList.filter((item) => {
